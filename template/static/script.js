...
function hasNotDrank(prediction) {
    // Start a 5-second timer
    timeoutId = setTimeout(() => {
        
        if(prediction.class == "Drinking" && !shouldBlur){
          resetTimeout();
        }else if (prediction.class === "Not Drinking" && shouldBlur) {
            console.log("Blur");
            document.body.style.filter = 'blur(20px)';
            
            hasNotDrank(prediction);   
        } else {
            //console.log("Condition changed or timer interrupted");
            resetTimeout(); 
        }
    }, 60*1000); 
}
